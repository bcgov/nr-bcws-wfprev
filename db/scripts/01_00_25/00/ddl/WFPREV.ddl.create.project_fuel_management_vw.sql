...
select p.project_guid, p.project_type_code, ptc.description as project_type_description,
	   ...
	   p.fire_centre_org_unit_id, fc.org_unit_name as fire_centre_org_unit_name, -- todo does not match
	   ppf.business_area_comment,
	   p.site_unit_name as  planning_unit_name,
	   p.total_actual_project_size_ha as groos_project_area_ha,
	   ...
	   (SELECT SUM(filter_section_score) FROM wfprev.eval_criteria_sect_summ ecss
		     ...
		     JOIN wfprev.evaluation_criteria ec       ON ec.evaluation_criteria_guid = ecsel.evaluation_criteria_guid AND ec.project_type_code = 'FUEL_MGMT'
		     WHERE ecss.eval_criteria_summary_guid = ecs.eval_criteria_summary_guid AND ecss.eval_criteria_sect_code = 'COARSE_FLT'
	   )  as total_coarse_filter_section_score,
   	   (SELECT SUM(filter_section_score) FROM wfprev.eval_criteria_sect_summ ecss 
     		JOIN wfprev.eval_criteria_selected ecsel ON ecsel.eval_criteria_sect_summ_guid = ecss.eval_criteria_sect_summ_guid AND ecsel.evaluation_criteria_select_ind = true
		    JOIN wfprev.evaluation_criteria ec       ON ec.evaluation_criteria_guid = ecsel.evaluation_criteria_guid AND ec.project_type_code = 'FUEL_MGMT'
		WHERE ecss.eval_criteria_summary_guid = ecs.eval_criteria_summary_guid AND ecss.eval_criteria_sect_code = 'MEDIUM_FLT'
	   ...
	   (SELECT string_agg(filter_section_comment, '| ') FROM wfprev.eval_criteria_sect_summ ecss 
		...
		WHERE ecss.eval_criteria_summary_guid = ecs.eval_criteria_summary_guid AND ecss.eval_criteria_sect_code = 'MEDIUM_FLT'
   	   ...
   	   (SELECT SUM(filter_section_score) FROM wfprev.eval_criteria_sect_summ ecss 
     		JOIN wfprev.eval_criteria_selected ecsel ON ecsel.eval_criteria_sect_summ_guid = ecss.eval_criteria_sect_summ_guid AND ecsel.evaluation_criteria_select_ind = true
		    JOIN wfprev.evaluation_criteria ec       ON ec.evaluation_criteria_guid = ecsel.evaluation_criteria_guid AND ec.project_type_code = 'FUEL_MGMT'
		WHERE ecss.eval_criteria_summary_guid = ecs.eval_criteria_summary_guid AND ecss.eval_criteria_sect_code = 'FINE_FLT'
	   ...
	   (SELECT SUM(filter_section_score) FROM wfprev.eval_criteria_sect_summ ecss 
     		JOIN wfprev.eval_criteria_selected ecsel ON ecsel.eval_criteria_sect_summ_guid = ecss.eval_criteria_sect_summ_guid AND ecsel.evaluation_criteria_select_ind = true
		    JOIN wfprev.evaluation_criteria ec       ON ec.evaluation_criteria_guid = ecsel.evaluation_criteria_guid AND ec.project_type_code = 'FUEL_MGMT'
		WHERE ecss.eval_criteria_summary_guid = ecs.eval_criteria_summary_guid AND ecss.eval_criteria_sect_code IN ( 'FINE_FLT', 'MEDIUM_FLT', 'COARSE_FLT' )
