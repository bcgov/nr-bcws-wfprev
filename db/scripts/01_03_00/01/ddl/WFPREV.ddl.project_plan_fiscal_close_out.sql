CREATE TABLE "wfprev"."project_plan_fiscal_close_out"
(
	"project_plan_fiscal_close_out_guid" UUID NOT NULL,    -- project_plan_fiscal_close_out_guid is a unique identifier for the record.
	"project_plan_fiscal_guid" UUID NOT NULL,    -- project_plan_fiscal_guid: Foreign key to project_plan_fiscal. This references both the project and the specific fiscal year, following the pattern of project_plan_fiscal_perf.
...
COMMENT ON TABLE "wfprev"."project_plan_fiscal_close_out"
	IS 'Project Plan Fiscal Close Out is used to record the final outcomes and comments when a project''s fiscal year plan is closed out.'
;

COMMENT ON COLUMN "wfprev"."project_plan_fiscal_close_out"."project_plan_fiscal_close_out_guid"
	IS 'project_plan_fiscal_close_out_guid is a unique identifier for the record.'
;

COMMENT ON COLUMN "wfprev"."project_plan_fiscal_close_out"."project_plan_fiscal_guid"
...
COMMENT ON COLUMN "wfprev"."project_plan_fiscal_close_out"."outcome_comment"
	IS 'Outcome Comment is used to document the final outcomes and comments for the fiscal year close-out.'
;

COMMENT ON COLUMN "wfprev"."project_plan_fiscal_close_out"."revision_count"
...
COMMENT ON COLUMN "wfprev"."project_plan_fiscal_close_out"."create_user"
	IS 'CREATE_USER is an audit column that indicates the user that created the record.'
;

COMMENT ON COLUMN "wfprev"."project_plan_fiscal_close_out"."create_date"
	IS 'CREATE_DATE is the date and time the row of data was created.'
;

COMMENT ON COLUMN "wfprev"."project_plan_fiscal_close_out"."update_user"
	IS 'UPDATE_USER is an audit column that indicates the user that updated the record.'
;

COMMENT ON COLUMN "wfprev"."project_plan_fiscal_close_out"."update_date"
	IS 'UPDATE_DATE is the date and time the row of data was updated.'
;

/* Create Primary Keys, Indexes, Uniques, Checks */

ALTER TABLE "wfprev"."project_plan_fiscal_close_out" ADD CONSTRAINT "ppfyco_pk"
	PRIMARY KEY ("project_plan_fiscal_close_out_guid")
;

-- Adding a unique constraint to ensure only one close-out record per project fiscal plan
ALTER TABLE "wfprev"."project_plan_fiscal_close_out" ADD CONSTRAINT "ppfyco_uk" UNIQUE ("project_plan_fiscal_guid")
;

CREATE INDEX "ppfyco_prjpfy_idx" ON "wfprev"."project_plan_fiscal_close_out" ("project_plan_fiscal_guid" ASC)
;

/* Create Foreign Key Constraints */

ALTER TABLE "wfprev"."project_plan_fiscal_close_out" ADD CONSTRAINT "ppfyco_prjpfy_fk"
