...
uuid_generate_v5(
    ...
    concat_ws('|',
      ...
      coalesce(ppf.project_fiscal_description, 'NULL')
       ...
       (SELECT SUM(CASE WHEN outstanding_obligations_ind THEN 1 ELSE 0 END)||'/'||COUNT(*)
        FROM wfprev.activity act
        WHERE act.project_plan_fiscal_guid = ppf.project_plan_fiscal_guid
       ) AS outstanding_obligations,
       (SELECT SUM(CASE WHEN carry_forward_ind THEN 1 ELSE 0 END)||'/'||COUNT(*)
        FROM wfprev.activity act
        WHERE act.project_plan_fiscal_guid = ppf.project_plan_fiscal_guid
       ) AS carried_forward,
       CASE
...
FROM wfprev.project p
  ...
  LEFT JOIN LATERAL (
      SELECT *
      FROM wfprev.project_plan_fiscal_closeout sub
      WHERE sub.project_plan_fiscal_guid = ppf.project_plan_fiscal_guid
