...
export class SearchFilterComponent implements OnInit {
  ...
  private updateForestDistricts(selectedRegions: string[]): void {
    const districtSelect = this.selects.find(
  ...
  private syncSelectedDistricts(options: Option[]): void {
    ...
    this.selectedForestDistrict = this.selectedForestDistrict.filter((v) =>
  ...
  assignDefaultFiscalYear(emit: boolean = true): void {
    ...
    const options =
      this.selects.find((item) => item.model === 'selectedFiscalYears')
        ?.options || [];

    const currentFiscalExists = options.some(
      (opt) => opt.value === fiscalYearValue,
    );
