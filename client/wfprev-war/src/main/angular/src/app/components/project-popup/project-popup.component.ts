...
export class ProjectPopupComponent implements OnInit {
  ...
  loadCodeTables(): void {
    ...
    this.codeTableService.getPlanFiscalStatusCodes().subscribe({
      next: (data) => {
        this.planFiscalStatusCode = data;
      },
      error: (err) =>
        console.error('Error fetching planFiscalStatusCodes', err),
    });

    this.codeTableService.getActivityCategoryCodes().subscribe({
      next: (data) => {
        this.activityCategoryCodes = data;
      },
      error: (err) =>
        console.error('Error fetching activityCategoryCodes', err),
    });
  }
  ...
  getCodeDescription(controlKey: string, value: any): string | null {
    switch (controlKey) {
      case CodeTableKeys.PROJECT_TYPE_CODE:
        return (
          this.projectTypeCode.find((item) => item.projectTypeCode === value)
            ?.description ?? null
        );

      case CodeTableKeys.PROGRAM_AREA_GUID:
        return (
          this.programAreaCode.find((item) => item.programAreaGuid === value)
            ?.programAreaName ?? null
        );

      case CodeTableKeys.ACTIVITY_CATEGORY_CODE:
        return (
          this.activityCategoryCodes.find(
